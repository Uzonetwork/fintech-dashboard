...
export default function TransactionsTable() {
  ...
  const columns = useMemo(
    () => [
      ...
      { accessorKey: "category", header: "Category" },
      {
        ...
        header: "Amount",
        cell: (info) => (
          <span className={info.getValue() < 0 ? "text-red-600" : "text-green-600"}>
            ${info.getValue()}
          </span>
        ),
      },
  ...
  const exportPDF = async () => {
    ...
    autoTable(doc, { startY: 22, head, body });
  ...
  if (!transactions.length)
    return <div className="bg-white p-6 rounded shadow">No transactions found.</div>;
